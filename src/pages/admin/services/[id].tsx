...
export default function AdminServiceEditPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage({ type: "error", text: error.response?.data?.message || "Failed to update service." });
      setIsUploading(false);
    } finally {
      setIsSaving(false);
    }
  ...
  if (isLoading) return <div className="text-xs font-mono font-bold uppercase tracking-widest animate-pulse p-10 text-primary-light">Loading Service Data...</div>;
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-12 pb-20 font-sans">
      ...
      <div className="flex items-center justify-between border-b border-slate-800 pb-8 relative">
         ...
         <h1 className="text-3xl font-extrabold text-white uppercase tracking-tighter font-syne m-0">Edit Service</h1>
      </div>
              ...
              {(imagePreview || existingImage) ? (
                 <div className="relative aspect-video border border-slate-800 overflow-hidden bg-slate-950/60 flex items-center justify-center">
                    ...
                    <button 
                      ...
                      className="absolute top-2 right-2 bg-red-600/80 p-2 text-white border-0 cursor-pointer text-xs font-mono flex items-center gap-2"
                    >
...
AdminServiceEditPage.getLayout = function getLayout(page: React.ReactElement) {
